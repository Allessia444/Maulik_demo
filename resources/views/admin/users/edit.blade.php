		...
		<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
			...
			<div class="form-group row">
				...
				<div class="col-sm-12 col-md-10">
					<input type="radio" name="gender" checked value="male" @if($user_profile->gender == "male") checked  @endif>Male
					<input type="radio" name="gender" value="female" @if($user_profile->gender == "female") checked  @endif>Female
				</div>
			</div>
			{!! Former::select('marrital_status')->options(['single'=>'single','married'=>'married'])->select($user_profile->marrital_status) !!}
			{!! Former::select('designation_id')->options($designations)->select($user->designation_id) !!}
			{!! Former::select('department_id')->options($departments)->select($user->department_id) !!}
